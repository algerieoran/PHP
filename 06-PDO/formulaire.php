...
// 3- Traitement du formulaire :
    if(!empty($_POST)) {  // si "$_POST" est rempli c'est que le formulaire a été soumis.
        ...
        // Contrôles sur le formulaire :
            if (!isset($_POST['prenom']) || strlen($_POST['prenom']) < 3 || strlen($_POST['prenom']) > 20) $message .= '<div>Le prénom doit comporter entre 3 et 20 caractères.</div>';  // on verifie si l'indice "prenom" existe bien, s'il n'existe pas on met un message à l'internaute . On vérifie aussi sa longeur qui doit être comprise entre 3 et 20 caractères.
            ...
            if (!isset($_POST['salaire']) || !is_numeric($_POST['salaire'])) $message .= '<div>Le salaire doit etre un nombre.</div>';
    ...
    function validateDate($date, $format ='d-m-Y') {  // $format ='d-m-Y' permet de donner une valeur par défaut au paramètre $format si on ne lui passe pas d'argument lors de l'appel de la fonction
       ...
       $d = DateTime::createFromFormat($format, $date);  // crée un objet date si la date est valide et  qu'elle correspond au format indiqué dans $format. Dans le cas contraire, retourne false (c'estv à dire si la date n'est pas valide ou qu'elle ne correspond pas au format indiqué)

       if ($d && $d->format($format) == $date) {  // si $d n'est pas false (voir ci-dessus) et que l'objet date $d est bien égale à la date $date , c'est qu'il n'y a pas eu d'extrapolation sur la date : exemple de 32/01/2010 qui devient 01/02/2010. Dans ce cas la date est valisé : on retourne true.
           return true;
       }else {
           return false;
       }
    ...
    if(empty($message)) {  // si $message est vide c'est qu'il n'y a pas d'erreur.
        ...
        if ($req) {
            ...
            $message .= '<div>Une érreue est survenue lors de l\'enregistrement.</div>';
        }
