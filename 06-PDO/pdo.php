...
$resultat = $pdo->exec("INSERT INTO employes (prenom, nom, sexe, service, date_embauche, salaire) 
                        ...
                        // exect() est utilisé pour la formulation de requête ne retournant pas de jeu de résultats : INSERT, DELETE et UPDATE.
...
// $result est le résultat de la requête sous une forme inexpoitable directement : il faut donc le transformer avec la méthode fetch() :
...
$result = $pdo->query("SELECT * FROM employes WHERE prenom = 'daniel'");
$employe = $result->fetch(PDO::FETCH_OBJ);  // transforme en un objet avec les noms des champs de la table comme propriétésde cet objet
debug($employe);
echo $employe->prenom . '<br>'; 

// Attention : après une requête, il faut choisir l'un des fetch(). Si l'on veut en refaire un, il faut refaire la requête : en effet, on ne peut pas effectuer plusieurs transformation successives sur le mêem objet $result.


//------------------
// Exercice : afficher le service de l'employé dont l'id_employes est 417 (production).
...
//----------------
// 04 - fetch() avec une boucle while (plusieurs résultats)
//----------------
...
while($employe = $resultat->fetch(PDO::FETCH_ASSOC)) {  // fetch retourne la ligne suivante du jeu de résultats en un array associatif. La boucle while permet de faire avancer le curseur dans le jeu de résultat et s'arrête quand le curseur est à la fin
...
$resultat = $pdo->query("SELECT DISTINCT service FROM employes");  // fonctionne aussi ici avec GROUP BY , mais ce dernier est plutôt utilisé avec les fonctions d'agrégats (assemblage) comme SUM(), COUNT(), MIN(), MAX() et AVG()
...
echo $donnees['prenom'] . ' ' . $donnees['nom'] . ' du servive ' . $donnees['service'] . '.';
